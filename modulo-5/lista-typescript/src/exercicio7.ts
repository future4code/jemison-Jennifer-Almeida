...
de organização de estoque da empresa. A pessoa desenvolvedora anterior a você chegou a criar uma função 
...
const listPrice = (array: Product[]) => {
    array.map((Produto) => {
        const novoUnitario = ajustaPreco(Produto.valorUnitario)
        Produto.valorUnitario = novoUnitario
        console.log(Produto)
    })
}

listPrice(listProducts)